...
ValuedAction DESPOT::OptimalAction(VNode* vnode) {
	...
	for (ACT_TYPE action = 0; action < vnode->children().size(); action++) {
		...
		// logi << "A STAR VALUE" << astar.value << endl;
		// logi << "LOWER BOUND VALUE" << qnode->lower_bound() << endl;
		if (qnode->lower_bound() > astar.value) {
			logi << "LOWER BOUND ACTION " << qnode->lower_bound() << endl;
			logi << "ASTAR ACTION " << astar.value << endl;
			logi << "USING LOWER BOUND ACTION" << endl;
			astar = ValuedAction(action, qnode->lower_bound());
		}
	}

	if (vnode->default_move().value > astar.value) {
		logi << "USING DEFAULT MOVE \n\n\n\n" << endl;
