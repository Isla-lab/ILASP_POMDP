			...
			{ { E_HELP, 0, "", "help", option::Arg::None,
					...
					{ E_NUMBER, 0, "", "number", option::Arg::Required,
							"  \t--number <arg>  \tNumber of elements of a problem (problem "
									"specific)." },
					{ E_prob_food, 2, "", "prob_food", option::Arg::Optional,
							"  \t--prob_food <arg>  \tNumber of prob_food ("
									"only for pacman)." },
					{ E_prob_ghosts, 2, "", "prob_ghosts", option::Arg::Optional,
							"  \t--prob_ghosts <arg>  \tNumber of prob_ghosts ("
									"only for pacman)." },
					...
					{ E_SAVEFILE, 0, "", "save", option::Arg::Required,
							"  \t--save  \tChoose output file to save final statistics." },
...
void PlannerBase::OptionParse(option::Option *options, int &num_runs,
	...
	if (options[E_SAVEFILE])
		Globals::config.savefile = options[E_SAVEFILE].arg;
...
void PlannerBase::PrintResult(int num_runs, Logger *logger,
	...
	if (Globals::config.savefile != ""){
		ofstream myfile;
		myfile.open(Globals::config.savefile, ios_base::app);
		myfile << "Total time: Real / CPU = "
