...
double Logger::AverageNumSteps() const {
	double sum = 0;
	for (int i = 0; i < num_step_.size(); i++) {
		double ns = num_step_[i];
		sum += ns;
	}
	return num_step_.size() > 0 ?
			(sum / num_step_.size()) : 0.0;
}

double Logger::AverageTimePerStep() const {
	double sum = 0;
	for (int i = 0; i < time_per_step_.size(); i++) {
		double tps = time_per_step_[i];
		sum += tps;
	}
	return time_per_step_.size() > 0 ?
			(sum / time_per_step_.size()) : 0.0;
}
...
double Logger::StderrTimePerStep() const {
	double sum = 0, sum2 = 0;
	for (int i = 0; i < time_per_step_.size(); i++) {
		double tps = time_per_step_[i];
		sum += tps;
	...
	int n = time_per_step_.size();
	return n > 0 ? sqrt(sum2 / n / n - sum * sum / n / n / n) : 0.0;
}

double Logger::StderrNumSteps() const {
	double sum = 0, sum2 = 0;
	for (int i = 0; i < num_step_.size(); i++) {
		double ns = num_step_[i];
		sum += ns;
...
bool Logger::SummarizeStep(int step, int round, bool terminal, ACT_TYPE action,
	...
	*out_ << "-----------------------------------Round " << round << " Step "
	...
	time_per_step_.push_back(time_per_step);
	*out_ << "- Time per step = " << time_per_step << endl;
...
void Logger::PrintStatistics(int num_runs) {
	if (Globals::config.savefile != ""){
		ofstream myfile;
		myfile.open (Globals::config.savefile);
		myfile << "\nCompleted " << num_runs << " run(s).\n";
		myfile << "Average total discounted reward (stderr) = "
				<< AverageDiscountedRoundReward() << " ("
				<< StderrDiscountedRoundReward() << ")\n";
		myfile << "Average total undiscounted reward (stderr) = "
				<< AverageUndiscountedRoundReward() << " ("
				<< StderrUndiscountedRoundReward() << ")\n";
		myfile << "Average num steps (stderr) = "
				<< AverageNumSteps() << " ("
				<< StderrNumSteps() << ")\n";
		myfile << "Average time per step (stderr) = "
				<< AverageTimePerStep() << " ("
				<< StderrTimePerStep() << ")\n";
		...
		cout << "Average num steps (stderr) = "
				<< AverageNumSteps() << " ("
				<< StderrNumSteps() << ")" << endl;
		cout << "Average time per step (stderr) = "
				<< AverageTimePerStep() << " ("
				<< StderrTimePerStep() << ")" << endl;
	}
}
