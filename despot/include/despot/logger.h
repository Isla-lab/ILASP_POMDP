...
protected:
	...
	std::vector<double> time_per_step_;
...
public:
	...
	virtual bool SummarizeStep(int step, int round, bool terminal, ACT_TYPE action,
			OBS_TYPE obs, double step_start_t, double step_time);
	...
	double AverageTimePerStep() const;
	double StderrTimePerStep() const;
	double AverageNumSteps() const;
	double StderrNumSteps() const;
