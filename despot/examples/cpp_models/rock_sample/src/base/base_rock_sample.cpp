...
void BaseRockSample::InitGeneral() {
    Coord rocks[num_rocks_];
    int range_x = num_rocks_ / 2;
    int range_y = 2;
    int count = 0;
    for (int i = 0; i < range_x; i++) {
        for (int j = 0; j < range_y; j++) {
            int x = ((unif_dist(random_state)*(size_/range_x-1)) + i*size_/range_x);
            int y = ((unif_dist(random_state)*(size_/range_y-1)) + j*size_/range_y);
...
void BaseRockSample::Init_12_4() {
	// Equivalent to RockSample_11_11.pomdp(x)
	cout << "Using special layout for rocksample(12, 4)" << endl;

	Coord rocks[] = { Coord(0, 8), Coord(5, 2), Coord(1, 4), Coord(5, 5) };

	start_pos_ = Coord(0, 6);
	grid_.SetAllValues(-1);
	for (int i = 0; i < num_rocks_; ++i) {
		grid_(rocks[i]) = i;
		rock_pos_.push_back(rocks[i]);
...
State* BaseRockSample::CreateStartState(string type) const {
	//TO FIX NUMBER OF GOOD ROCKS
	// srand(time(0));
	// vector<int> distribution(num_rocks_);
	// iota(distribution.begin(), distribution.end(), 0);
	// random_shuffle(distribution.begin(), distribution.end());
    // std::vector<int> val_rocks(distribution.begin(), distribution.begin()+num_rocks_/2);
	// int state = (1 << num_rocks_) * CoordToIndex(start_pos_);
	// for (int i = 0; i < num_rocks_; i++) {
	// 	if (std::find(val_rocks.begin(), val_rocks.end(), i) != val_rocks.end()) SetFlag(state, i);
	// }
	...
	ValuedAction Value(const vector<State*>& particles, RandomStreams& streams,
		...
		for (int rock = 0; rock < rs_model_->num_rocks_; rock++){
			if (expected_sampling_value[rock] / total_weight > 0.0)
				SetFlag(state, rock);
			// logi << "USING MMAP" << endl;
			// logi << "Rock " << rock << " has value " << expected_sampling_value[rock] << endl;
		}
...
public:
	...
	ValuedAction Value(const vector<State*>& particles, RandomStreams& streams,
		...
		value += 0 * weight * discount * Globals::Discount(grid_.xsize() - rob_pos.x);
...
class RockSampleRulesDefaultPolicy11 : public DefaultPolicy {
protected:
	const BaseRockSample* rs_model_;
	mutable vector<ACT_TYPE> preferred_;
	mutable vector<ACT_TYPE> legal_;
public:
	RockSampleRulesDefaultPolicy11(const BaseRockSample* model, ParticleLowerBound* bound) :
	...
	ACT_TYPE Action(const vector<State*>& particles, RandomStreams& streams,
		...
		// const RockSampleState& rockstate =
		// 	static_cast<const RockSampleState&>(particles[0]);
		Coord rob_pos = rs_model_->GetRobPos(particles[0]);
		legal_.push_back(Compass::EAST);
		legal_.push_back(rs_model_->E_EXIT);
		if (rob_pos.y < rs_model_->grid_.ysize() - 1) legal_.push_back(Compass::NORTH);
		if (rob_pos.y > 0) legal_.push_back(Compass::SOUTH);
		if (rob_pos.x > 0) legal_.push_back(Compass::WEST);
		...
		for (int t = 0; t < history.Size(); t++) {
			ACT_TYPE action = history.Action(t);

			if (action < rs_model_->E_SAMPLE) {
				Coord next;
				if (action<rs_model_->E_EXIT) next = pos + Compass::DIRECTIONS[action];
				else next = pos + Compass::DIRECTIONS[Compass::EAST];
		...
		// logi << "HISTORY SIZE " << history.Size() << endl;

		//GET STATUS OF ROCKS
		vector<double> expected_sampling_value = vector<double>(
			rs_model_->num_rocks_);
		vector<int> integer_expected_sampling_value = vector<int>(
			rs_model_->num_rocks_);
		int state = 0;
		// Coord rob_pos(-1, -1);
		double total_weight = 0;
		for (int i = 0; i < particles.size(); i++) {
			...
			for (int r = 0; r < rs_model_->num_rocks_; r++) {
				expected_sampling_value[r] += particle->weight
					* (CheckFlag(particle->state_id, r) ? 1 : 0);
				// logi << "check flag " << CheckFlag(particle->state_id, r) << endl;
			}
			total_weight += particle->weight;
		}


		// std::vector<BaseRockSample::Target> targets;
		// targets.clear();
		std::vector<ValuedAction> actions = std::vector<ValuedAction>(rs_model_->num_rocks_);

		for (int rock = 0; rock < rs_model_->num_rocks_; rock++){
			// logi << "Rock " << rock << " has value " << expected_sampling_value[rock]/total_weight << endl;
			integer_expected_sampling_value[rock] = expected_sampling_value[rock]/total_weight * 100;
			integer_expected_sampling_value[rock] -= (integer_expected_sampling_value[rock]%10);
			// logi << "Rock " << rock << " has INTEGER value " << integer_expected_sampling_value[rock] << endl;
			
			if (!sampled[rock]){
				legal_.push_back(rs_model_->E_SAMPLE + rock + 1);
				Coord rock_pos = rs_model_->rock_pos_[rock];
				dist = Coord::ManhattanDistance(rock_pos, rob_pos);
				if (dist<=0) {
					if (integer_expected_sampling_value[rock]<=80) preferred_.push_back(rs_model_->E_SAMPLE + rock + 1);
					legal_.push_back(rs_model_->E_SAMPLE);
				}
				if (dist<=1){
					// BaseRockSample::Target target;
					// target.val = integer_expected_sampling_value[rock];
					// target.dist = dist;
					// target.rock = rock;
					// target.X = rock_pos.x;
					// target.Y = rock_pos.y;
					// targets.push_back(target);
					if (integer_expected_sampling_value[rock]>=90) preferred_.push_back(rs_model_->E_SAMPLE);
					if (rock_pos.x - rob_pos.x >= 1)
						preferred_.push_back(Compass::EAST);
					if (rock_pos.x - rob_pos.x <= -1)
						preferred_.push_back(Compass::WEST);
					if (rock_pos.y - rob_pos.y >= 1)
						preferred_.push_back(Compass::NORTH);
					if (rock_pos.y - rob_pos.y <= -1)
						preferred_.push_back(Compass::SOUTH);
				}
			}
			else num_sampled++;
		}
		// logi << "Value of rocks" << endl;
		// for (auto i=0; i < integer_expected_sampling_value.size(); i++) 
		// 	logi << "Rock " << i << " has value " << integer_expected_sampling_value[i] << endl;
		
		// sort(targets.begin(), targets.end(), rs_model_->compare_targets);
		// for (auto i=0; i<targets.size(); ++i)
		// {
		// 	BaseRockSample::Target best_target = targets[i];
		// 	if (best_target.val>=90 and best_target.dist<=0)
		// 		preferred_.push_back(rs_model_->E_SAMPLE);
		// 	if (best_target.val<=50) 
		// 		preferred_.push_back(rs_model_->E_SAMPLE + best_target.rock + 1);
		// 	if (best_target.X - rob_pos.x >= 1)
		// 		preferred_.push_back(Compass::EAST);
		// 	if (best_target.X - rob_pos.x <= -1)
		// 		preferred_.push_back(Compass::WEST);
		// 	if (best_target.Y - rob_pos.y >= 1)
		// 		preferred_.push_back(Compass::NORTH);
		// 	if (best_target.Y - rob_pos.y <= -1)
		// 		preferred_.push_back(Compass::SOUTH);
		...
		//exit
		double num_rocks_double = rs_model_->num_rocks_;
		...
		// logi << "PERC OF SAMPLED ROCKS IS " << num_sampled_percentage << endl;
		if (num_sampled_percentage >= 50){
			preferred_.push_back(rs_model_->E_EXIT);
		}

		if (preferred_.size() != 0){
			std::vector<double> prob_actual;
			for (auto a : preferred_){
				if (a<6) prob_actual.push_back(rs_model_->prob_actions[a]);
				else prob_actual.push_back(rs_model_->prob_actions[6]);
			}
		...
		else return legal_[Random::RANDOM.NextInt(legal_.size())];
	}
};
...
class RockSampleRulesDefaultPolicy : public DefaultPolicy {
protected:
	const BaseRockSample* rs_model_;
	mutable vector<ACT_TYPE> preferred_;
	mutable vector<ACT_TYPE> legal_;
public:
	RockSampleRulesDefaultPolicy(const BaseRockSample* model, ParticleLowerBound* bound) :
		DefaultPolicy(model, bound),
		rs_model_(model) {
	}
	...
	ACT_TYPE Action(const vector<State*>& particles, RandomStreams& streams,
		...
		for (int rock = 0; rock < rs_model_->num_rocks_; rock++){
			...
			if (!sampled[rock]){
				...
				if ((integer_expected_sampling_value[rock]>=70 && integer_expected_sampling_value[rock]<=80) || dist<=1){
					BaseRockSample::Target target;
					target.val = integer_expected_sampling_value[rock];
					target.dist = dist;
					target.rock = rock;
		...
		if (num_sampled_percentage >= 25){
			for (int i=0; i<rs_model_->num_rocks_; ++i){
				if (!sampled[i]){
					dist = Coord::ManhattanDistance(rob_pos, rs_model_->rock_pos_[i])
;					if ((integer_expected_sampling_value[i]<=40) || (dist<=8 && dist >=5)){
						preferred_.push_back(rs_model_->E_EXIT);
						break;
					} 
				}
...
class RockSampleRules60DefaultPolicy : public DefaultPolicy {
protected:
	const BaseRockSample* rs_model_;
	mutable vector<ACT_TYPE> preferred_;
	mutable vector<ACT_TYPE> legal_;
public:
	RockSampleRules60DefaultPolicy(const BaseRockSample* model, ParticleLowerBound* bound) :
		DefaultPolicy(model, bound),
		rs_model_(model) {
	}
	...
	ACT_TYPE Action(const vector<State*>& particles, RandomStreams& streams,
		...
		for (int rock = 0; rock < rs_model_->num_rocks_; rock++){
			...
			if (!sampled[rock]){
				...
				if (integer_expected_sampling_value[rock]<=80){
					BaseRockSample::Target target;
					target.val = integer_expected_sampling_value[rock];
					target.dist = dist;
					target.rock = rock;
...
class RockSampleRules40DefaultPolicy : public DefaultPolicy {
protected:
	const BaseRockSample* rs_model_;
	mutable vector<ACT_TYPE> preferred_;
	mutable vector<ACT_TYPE> legal_;
public:
	RockSampleRules40DefaultPolicy(const BaseRockSample* model, ParticleLowerBound* bound) :
		DefaultPolicy(model, bound),
		rs_model_(model) {
	}
	...
	ACT_TYPE Action(const vector<State*>& particles, RandomStreams& streams,
		...
		if (num_sampled_percentage >= 25){
			for (int i=0; i<rs_model_->num_rocks_; ++i){
				if (!sampled[i]){
					dist = Coord::ManhattanDistance(rob_pos, rs_model_->rock_pos_[i])
;					if (integer_expected_sampling_value[i]>=80){
						preferred_.push_back(rs_model_->E_EXIT);
						break;
					} 
				}
...
class RockSampleRules20DefaultPolicy : public DefaultPolicy {
protected:
	const BaseRockSample* rs_model_;
	mutable vector<ACT_TYPE> preferred_;
	mutable vector<ACT_TYPE> legal_;
public:
	RockSampleRules20DefaultPolicy(const BaseRockSample* model, ParticleLowerBound* bound) :
		DefaultPolicy(model, bound),
		rs_model_(model) {
	}
	...
	ACT_TYPE Action(const vector<State*>& particles, RandomStreams& streams,
		...
		for (int rock = 0; rock < rs_model_->num_rocks_; rock++){
			...
			if (!sampled[rock]){
				...
				if ((integer_expected_sampling_value[rock]<=80) || dist<=0){
					BaseRockSample::Target target;
					target.val = integer_expected_sampling_value[rock];
					target.dist = dist;
					target.rock = rock;
...
ScenarioLowerBound* BaseRockSample::CreateScenarioLowerBound(string name, string
	...
	if (name == "TRIVIAL") {
		...
		// return new RockSampleRulesLowerBound(this);
		return new RockSampleRulesDefaultPolicy(this, CreateParticleLowerBound(particle_bound_name));
	} else if (name == "RULES_BAD") {
		// return new RockSampleRulesLowerBound(this);
		return new RockSampleRulesDefaultPolicy(this, CreateParticleLowerBound(particle_bound_name));
	} else if (name == "RULES20") {
		// return new RockSampleRulesLowerBound(this);
		return new RockSampleRules20DefaultPolicy(this, CreateParticleLowerBound(particle_bound_name));
	} else if (name == "RULES40") {
		// return new RockSampleRulesLowerBound(this);
		return new RockSampleRules40DefaultPolicy(this, CreateParticleLowerBound(particle_bound_name));
	} else if (name == "RULES60") {
		// return new RockSampleRulesLowerBound(this);
		return new RockSampleRules60DefaultPolicy(this, CreateParticleLowerBound(particle_bound_name));
	} else if (name == "RANDOM") {
	...
	void ComputePreference(const State& state) {
		...
		for (int t = 0; t < history_.Size(); t++) {
			...
			if (action > rs_model_->E_SAMPLE) {
				...
				measured[rock]++;
				counts[rock] += (obs == rs_model_->E_GOOD) ? 1 : 0;
		...
		legal_actions_.push_back(rs_model_->E_EXIT);
		...
		if (all_bad) {
			...
			preferred_actions_.push_back(rs_model_->E_EXIT);
			return;
		}
...
int BaseRockSample::NextState(int s, ACT_TYPE a) const {
	...
	if (a < E_SAMPLE) {
		if (a<E_EXIT) rob_pos += Compass::DIRECTIONS[a];
		else rob_pos += Compass::DIRECTIONS[Compass::EAST];
